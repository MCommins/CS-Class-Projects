...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    if(rightChild < max){
        ...
        else if(compare(getDynArr(heap, leftChild), getDynArr(heap, rightChild)) == 1){
            if(compare(getDynArr(heap, rightChild), getDynArr(heap, pos)) == -1){
...
void sortHeap(DynArr *heap)
{
   ...
   //Reversing array so first becomes last, vice-versa, and so on;
   for(int i = 0; i < sizeDynArr(heap) / 2; i++){
       swapDynArr(heap, i, sizeDynArr(heap) - i - 1);
   }
}
