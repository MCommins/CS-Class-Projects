...
const char *WINDOWTITLE = { "Michael Commins - CS 450 - Project1" };
...
const GLdouble torusInnerRad = 1.0;
const GLdouble torusOuterRad = 2.0;
...
int		ActiveButton;			// current button that is down				// list to hold the axes
...
int		HeliPerspective;		// INNER or OUTER
...
Display( )
{
	...
	if (HeliPerspective == INNER) {
		gluLookAt(0.3, 1.7, -4.9, 0., 1.5, -10., 0., 1., 0.);
	} else {
		gluLookAt(6., 6., 10., 0., 0., 0., 0., 1., 0.);
	}

	if (HeliPerspective == OUTER) {
		// rotate the scene:
	...
	glRotatef(360.f * Time * rotationSpeed, 0., 1., 0.);
	...
	glRotatef(360.f * Time * rotationSpeed * 3, 1., 0., 0.);
	glRotatef(90.f, 0., 1., 0.);
	glScalef(1.5, 1.5, 1.5);
	glCallList(BladeList);
	glPopMatrix();

	// Draw a Torus to be viewed
	glPushMatrix();
	glTranslatef(0.f, 3.f, -12.f);
	glutWireTorus(torusInnerRad, torusOuterRad, 10, 10);
	glPopMatrix();
...
InitMenus( )
{
	...
	glutAddMenuEntry("Outer", 1);
	glutAddMenuEntry("Inner", 0);
